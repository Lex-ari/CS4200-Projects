...
class Chessboard(tk.Tk):
    ...
    def highlight_row(self, row):
        """Highlight the current row where a queen is placed by changing its color."""
        square_size = 50
        for col in range(8):
            # Set the highlight color to indicate placement
    ...
    def place_next_queen(self):
        """Place queens one by one with backtracking, and highlight the row where a queen is placed."""
    ...
    def search_next_move(self):
        ...
        if col >= 8:
            ...
            if self.queens_positions:
                ...
                self.highlight_row(self.current_row)  # Highlight the row where the queen was removed
        ...
        if self.is_safe(self.current_row, col):
            ...
            # Highlight the row where the queen was placed
            ...
            if self.current_row == 8:
                ...
                self.trigger_confetti()  # Call the confetti method
                return  # Successfully placed all queens
        else:
            # If not safe, check the next column in the same row
            self.search_next_move()
    ...
    def create_confetti(self):
        """Create a single confetti piece at a random position on the canvas."""
        x = random.randint(0, 400)  # Canvas width
        y = random.randint(0, 400)  # Canvas height
        radius = random.randint(5, 10)  # Random size for confetti
        color = random.choice(["red", "blue", "green", "yellow", "purple", "orange"])  # Random color
        self.canvas.create_oval(
            x - radius,
            y - radius,
            x + radius,
        ...
        # Add a fade-out effect for the confetti
        self.after(200, self.fade_out_confetti, x, y, radius, color)

    def fade_out_confetti(self, x, y, radius, color):
        """Gradually fade out the confetti pieces."""
        for _ in range(10):  # Create multiple fade-out steps
            self.after(50 * _, lambda r=radius, xc=x, yc=y: self.canvas.delete(self.canvas.create_oval(
                xc - r, yc - r, xc + r, yc + r, fill=color, outline=color)))
