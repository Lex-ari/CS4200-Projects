    ...
    def search_next_move(self):
        ...
        if col >= 8:
            ...
            if self.queens_positions:
                ...
                self.highlight_row(self.current_row)  # Keep the highlight on the row where the queen was removed
        ...
        if self.is_safe(self.current_row, col):
            ...
            # Highlight the next row only if we are not done
            self.highlight_row(self.current_row)
        else:
            # Continue searching in the current row for a valid position
            self.place_next_queen()  # Search again
